...
class AppleStore extends Base implements CheckerInterface
{
    protected const SUCCESS = ['subscription' => 'expired'];
    ...
    public function CheckStatus($app)
    {
        ...
        if ($ans->ok()) {
            if (empty(current(array_diff($ans->json(), self::SUCCESS)))) {
                SubscriptionExpired::dispatch($app);
                parent::$counter++;
            }
        }
        else{
            RecheckStatus::dispatch($this->checker, AppleStore::class)->delay(
